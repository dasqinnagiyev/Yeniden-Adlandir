...
async def UploadFile(bot: Client, message: Message, file_path: str, file_size):
    try:
        ...
        sent_ = await bot.send_document(
            ...
            caption=((Config.CAPTION.format((await bot.get_me()).username) + f"\n\n**Faylın adı:** `{file_path.rsplit('/', 1)[-1]}`\n**Fayl ölçüsü:** `{humanbytes(file_size)}`") if (caption_ is None) else caption_),
            reply_markup=InlineKeyboardMarkup(
                [
                    [InlineKeyboardButton("Sahibim - @dasqinnagiyev", url="https://instagram.com/dasqinnagiyev")],
        ...
        await forward_.reply_text(
            text=f"**İstifadəçi:** [{message.chat.first_name}](tg://user?id={str(message.chat.id)})\n**İstifadəçi adı:** `{message.chat.username}`\n**UserID:** `{message.chat.id}`",
            disable_web_page_preview=True,
            quote=True
        )
    except Exception as err:
        try:
            await message.edit(f"Fayl verilə bilmir!\n**Xəta:**\n`{err}`")
            await asyncio.sleep(50)
        except:
            print(f"Fayl yüklənə bilmədi!\n**Xəta:** {err}")
...
async def UploadVideo(bot: Client, message: Message, file_path: str, file_size, width: int = 100, height: int = 100, duration: int = 0, default_thumb: Thumbnail = None):
    try:
        ...
        sent_ = await bot.send_video(
            ...
            width=width,
            height=height,
            caption=((Config.CAPTION.format((await bot.get_me()).username) + f"\n\n**Fayl adı:** `{file_path.rsplit('/', 1)[-1]}`\n**Fayl ölçüsü:** `{humanbytes(file_size)}`") if (caption_ is None) else caption_),
            reply_markup=InlineKeyboardMarkup(
                [
                    [InlineKeyboardButton("Shibim - @dasqinnagiyev", url="https://instagram.com/dasqinnagiyev")],
                    [InlineKeyboardButton("DAQO MODS", url="https://t.me/daqomods"),
                     InlineKeyboardButton("Maraqlı botlar", url="https://t.me/daqobots")]
                ]
            )
...
async def UploadAudio(bot: Client, message: Message, file_path: str, file_size, duration: int, title: str, performer: str):
    try:
        ...
        sent_ = await bot.send_audio(
            ...
            performer=(performer if (performer is not None) else "dasqinnagiyev"),
        ...
        forward_ = await sent_.forward(chat_id=Config.LOG_CHANNEL)
        await forward_.reply_text(
            text=f"**istifadəçi:** [{message.chat.first_name}](tg://user?id={str(message.chat.id)})\n**istifadəçi adı:** `{message.chat.username}`\n**UserID:** `{message.chat.id}`",
            disable_web_page_preview=True,
            quote=True
        )
    except Exception as err:
